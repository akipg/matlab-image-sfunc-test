...
#define S_FUNCTION_NAME                create_noise_image
...
/* Input Port  2 */
#define IN_PORT_2_NAME                 noise_size
...
extern void create_noise_image_Outputs_wrapper(const int32_T *width,
...
{
  const int32_T *width = (int32_T *) ssGetInputPortRealSignal(S, 0);
  const int32_T *height = (int32_T *) ssGetInputPortRealSignal(S, 1);
  const int32_T *noise_size = (int32_T *) ssGetInputPortRealSignal(S, 2);
  uint64_T *img_adr = (uint64_T *) ssGetOutputPortSignal(S, 0);
  create_noise_image_Outputs_wrapper(width, height, noise_size, img_adr);
}
